...
def refresh_session(payload: schemas.TokenRefreshRequest, db: Session = Depends(get_db)):
    ...
    # Validate against active Redis session whitelist
...
# --- UPDATED CRUD LOGIC supporting inline field text changes ---
...
def get_tasks(current_user: User = Depends(utils.get_current_user), db: Session = Depends(get_db)):
    ...
    tasks_json = [schemas.TaskResponse.from_orm(t).model_dump() for t in tasks]
